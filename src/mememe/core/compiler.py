...
且只画一个最关键的小道具，绝不铺开整个场景；若描述里出现重复或增殖的符号
（如多个问号、漫天纸片），只画其中一个、点到为止，不要让它们布满画面。
漫画特效（速度线、网点、拟声符号、排线）至多在角色周边少量点缀烘托情绪，
绝不能铺满或填满画面——大面积纯白留白必须保留，既是干净的贴纸观感，也便于
抠图和转成动图（满屏特效的复杂画面会让动图压不进体积上限）。
